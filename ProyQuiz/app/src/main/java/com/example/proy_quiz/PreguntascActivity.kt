...
class PreguntascActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Nombre=intent.getStringExtra(setDatac.nombre)
        val usuario=intent.getStringExtra(setDatac.nombre)
       // f_nombre.text="${usuario}, marca la opción correcta"
        //setData.getPregunta()


        preguntasList=setDatac.getPreguntac()
        setPregunta()
            ...
            if (selectOpc!=0){
                ...
                when{
                    currentPosition<=preguntasList!!.size->{
                        ...
                        intent.putExtra(setDatac.nombre,Nombre.toString())
                        intent.putExtra(setDatac.score,score.toString())
