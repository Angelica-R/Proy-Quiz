...
class PreguntaspActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Nombre=intent.getStringExtra(setDatap.nombre)
        val usuario=intent.getStringExtra(setDatap.nombre)
        //f_nombre.text="${usuario}, marca la opción correcta"
        //setData.getPregunta()


        preguntasList=setDatap.getPreguntap()
        setPregunta()
            ...
            if (selectOpc!=0){
                ...
                when{
                    currentPosition<=preguntasList!!.size->{
                        ...
                        intent.putExtra(setDatap.nombre,Nombre.toString())
                        intent.putExtra(setDatap.score,score.toString())
